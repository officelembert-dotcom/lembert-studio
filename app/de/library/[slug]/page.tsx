...
export async function generateStaticParams() {
  return getAllLibraryItems('de').map((item) => ({ slug: item.slug }))
}

export async function generateMetadata({ params }: { params: { slug: string } }) {
  const item = getLibraryItemBySlug(params.slug, 'de')
  if (!item) return {}
  return {
    title: `${item.title} — Bibliothek`,
    description: item.description,
    openGraph: { title: item.title, description: item.description },
  }
}

export default function LibraryItemPageDe({ params }: { params: { slug: string } }) {
  const item = getLibraryItemBySlug(params.slug, 'de')
  if (!item) notFound()
  ...
  return (
    <div className="page-enter mx-auto max-w-page px-6 md:px-10">
      ...
      <Link href="/de/library" className="font-inter text-birch no-underline" style={{ fontSize: '12px', opacity: 0.45 }}>
      ...
      {item.file && (
        ...
        >
          {isAudio ? 'MP3 herunterladen' : 'PDF herunterladen'}
        </a>
      )}
